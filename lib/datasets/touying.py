...
if __name__ == '__main__':
    ds = KittiRCNNDataset(root_dir='data/')
    i = 120
    print(ds[i]['pts_origin_xy'])
    img_file = os.path.join('data/KITTI/object/training/image_2', '%06d.png' % ds[i]['sample_id'])

    from PIL import Image, ImageDraw

    # draw = ImageDraw.Draw(img_file)
    xy = ds[i]['pts_origin_xy']
    ...
    pylab.imshow(img)  # 显示原图

    # 在图像上画上标价点，然后显示插完标记点的图像
    # row和clo是列表数据类型，row是横坐标集，clo是纵坐标集
    x = ds[i]['pts_origin_xy'][:, 0]
    y = ds[i]['pts_origin_xy'][:, 1]

    plot(x, y, 'o', markersize=0.3)
